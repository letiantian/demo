    ...
    public static void main(String[] args) throws SQLException, ClassNotFoundException {
        ...
        try {
            ...
            for (int i=1; i<=columnCount; ++i) {  // 注意，index是从1开始
                String columnLabel = tables.getMetaData().getColumnLabel(i);
                String columnClassName = tables.getMetaData().getColumnClassName(i);
            ...
            while (tables.next()) {
                System.out.println("第1种方式获取表名：" + tables.getString("Tables_in_bank"));
                System.out.println("第2种方式获取表名：" + tables.getString(1));
            }

            // 查询表的创建语句
            ResultSet createSql = stmt.executeQuery("SHOW CREATE TABLE `user_balance`");
            while (createSql.next()) {
                System.out.println("创建语句：" + createSql.getString(2)); // 创建语句在查询结果的第2列
            }
